"""LoCoMo benchmark runner — local mode, bypass HTTP server.

Usage:
  MAX_CONVERSATIONS=5 MAX_QUESTIONS=100 TOP_K=50 python3 run_locomo_local.py
...
from benchmarks.locomo.run import (
    ...
    load_evidence_lookup, DEFAULT_DATASET_DIR,
)
...
async def main():
    ...
    def save_checkpoint():
        ...
        CHECKPOINT_PATH.write_text(json.dumps({
            "metadata": {
                "benchmark": "locomo", "memory_system": "mneme",
                "model": DEEPSEEK_MODEL, "top_k": TOP_K,
                "cutoffs": CUTOFFS, "conversations": MAX_CONVERSATIONS,
                "max_questions": MAX_QUESTIONS, "mode": "local",
            },
    ...
    for conv_idx in range(min(MAX_CONVERSATIONS, len(dataset))):
        ...
        for qi, qa in conv_questions:
            ...
            # Generate answer
            # ⚠️ Field mapping: Memory.to_dict() uses "content" key,
            # but prompts typically read "memory". Must align.
            memories_for_prompt = [
                {"memory": m.to_dict().get("content", ""),
                 "created_at": m.to_dict().get("created_at", ""),
                 "id": m.to_dict().get("id", ""),
                 "score": s}
                for m, s in search_results[:50]
            ]
            gen_prompt = get_answer_generation_prompt(
                question, memories_for_prompt,
            ...
            judge_prompt = (
                ...
                ) if ev_ctx else get_judge_prompt(
            ...
            print(f"     Judge: {'✅ CORRECT' if correct else '❌ WRONG'} (score={score})")
    ...
    if all_evaluations:
        for label in [cutoff_label(c) for c in cutoffs]:
            ...
            correct = sum(
                ...
                if e["cutoff_results"][label]["score"] >= 0.5
            )
    ...
    path = results_dir / f"locomo_mneme_local_{ts}.json"
