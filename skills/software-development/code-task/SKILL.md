...
## 研究驱动开发模式（Research-Backed Dispatch）

> 当一个开发任务涉及**领域知识、业界方案对比、学术论文参考**时，不要直接派小弟。
> 先做研究，再写 prompt，让小弟的代码有据可依。
> 验证案例（2026-07-12 Mneme 补丁链）：搜了 EvoMem 论文 + agentmemory + Memoria + LWW-KO 等来源，
> 编译成 4 段关键发现 → 小弟一次 dispatch 产出 10 文件 823 行，ruff 零报错，201 tests pass。

### 场景识别

| 信号词 | 说明 |
|:-------|:------|
| "先找论文"、"搜一下相关资料"、"找论文和工程实践" | 用户明确要求先研究 |
| 涉及开放设计空间的新功能（多种方案可对比） | 需要 Emma 做调研给选项 |
| 从零构建一个系统（不是改已有代码） | 需要参考业界实践避免踩坑 |
| 用户说"有什么建议？"后选方向 | 需要 Emma 出研究简报再决策 |

### 标准流程

```
Phase A: 研究（Emma 做）
  ├─ 搜索论文（arXiv/Google Scholar）→ 提取关键发现
  ├─ 搜索工程实践（GitHub 同类项目）→ 提取实现模式
  └─ 编译为结构化摘要（按设计维度分节）
...
Phase C: 写 prompt（Emma 做）
  ├─ 设计背景段：研究发现的 key insight + 引用来源
  ├─ 改什么段：精确描述行为变化
  ├─ 参考文件段：需读取的现有源码路径
  └─ 约束+验证段：技术约束 + 可量化通过标准

Phase D: Dispatch（Emma 做）
  ├─ background + notify 派小弟
  ├─ 验证交付（diff + 测试 + PII）
  └─ 推远程
...
1. **EvoMem（arXiv 2606.13681）** — 补丁式记忆：记录非增量更新为结构化补丁，
   包含旧内容、新内容、更新原因、证据。检索时默认返回最新版本，当查询涉及
   变更历史时额外检索相关补丁。

2. **agentmemory** — Jaccard > 0.7 触发 supersede，版本链完整保留。
   关系类型：supersedes, extends, derives, contradicts, related。

3. **LWW-KO（Last-Write-Wins）** — 确定性覆盖语义：检索前过滤过期版本，
   多跳推理提升 +13pp。

### 核心设计区分

- **增量更新**（新增事实、追加标签）→ 直接更新，不生成 patch
- **非增量更新**（修改内容、改变含义）→ 生成 patch 记录
```

引用原则：
- **每篇只给 1-2 句核心发现** — 不看论文也能理解设计动机
- **附带量化锚点**（"+13pp"、"Jaccard > 0.7"）让小弟有明确的实现参考
- **区分"参考"和"照做"** — 有些是设计灵感，有些是可直接复用的模式
...
处理规则：
- **模型** → 一律 **`deepseek-v4-flash`**（V4-Flash 0731 正式版 Agent 能力大幅增强，无需再拆两模型）
...
判断标准：用户的话里包含分析类的信号词（"分析"/"看看"/"为什么"），但最终想要的产出是改好的代码（不是分析报告）。此时归为组合型，用 Flash 模型一次解决。

### 🔍 分析类任务

| 属性 | 值 |
|:----|:---|
| **模型** | **一律 `deepseek-v4-flash`**（分析类已统一走 Flash 正式版） |
...
| 场景 | 推荐模型 | 理由 |
|:-----|:---------|:-----|
| **复杂任务**（跨 3+ 文件、新模块、架构改动） | `opencode-go/deepseek-v4-flash` | 统一走 Flash 正式版（0731 后训练 Agent 能力大幅增强） |
| **中等任务**（单文件、逻辑有一定复杂度） | `opencode-go/deepseek-v4-flash` | 统一走 Flash 正式版 |
| **简单任务**（<30 行改、纯配置、测试补全） | `opencode-go/deepseek-v4-flash` | 低成本快速响应 |

如果用户明确指定了模型（如"用 GLM-5.2"），**无条件照做**，不要用上表覆盖用户的选择。

#### 🔴 复杂任务 → 统一 deepseek-v4-flash
...
统一 **`deepseek-v4-flash`**（2026-08-03 起：V4-Flash-0731 正式版已公测，Agent 能力大幅增强，官方基准远超 V4-Pro 预览版，所有路由统一走 Flash）。

> **规则变更记录（2026-08-03）：** 旧规则"拿不准默认用 Pro"已废止。原因：V4-Flash 0731 正式版通过专项后训练大幅增强代码修改/工具调用/多步骤任务能力，官方数据其 Agent 能力远超 V4-Pro 预览版，且 V4-Pro 正式版仍未发布（API 还是 4 月的预览版）。所以无论任务复杂度，一律 `deepseek-v4-flash`。
...
**如果有未提交的本地改动，必须决定怎么处理：**

| 改动类型 | 处理方式 |
|:---------|:---------|
| **机械清理**（ruff 修复、pytest 配置、.gitignore 等） | **先 commit 再 dispatch**。这些改动是独立的有价值的修复，不该被小弟覆盖历史 |
| **半截子的临时改动**（正在调试/探索中的代码） | **git stash**，dispatch 完成后 pop 回来 |
| **需要小弟一并实现的改动**（如：修改了设计文档，需要小弟按新设计实现代码） | **先 commit**，让小弟从最新状态开始工作 |
| **与本次 dispatch 无关的本地试验** | **git stash**，避免干扰小弟的 diff |

> ⚠️ **不处理未提交改动的后果：** 小弟从最后一个 commit 开始工作，commit 时不会包含你未提交的改动。你之前手动修的文件会停留在工作区，既不进小弟的 commit，也与小弟改过的新版本文件产生冲突——导致"一半修了、一半没修"的混乱状态。**必须 commit 或 stash，不要留脏工作区就 dispatch。**
>
...
>
> **教训（2026-07-12 Mneme 补丁链）：** Emma 先手动修了 ruff 问题和 pytest 配置（未 commit），然后 dispatch 小弟做补丁链。小弟从 a03496e 开始工作，commit 了 3 个补丁链 commit，但之前的 ruff 修复被排除在 commit 之外——仍停留在工作区，与小弟改过的新版本文件共存，需要手动处理合并。

### 调用格式

```bash
# 所有任务统一 Flash（V4-Flash-0731 正式版）
