...
| 场景 | 推荐模型 | 理由 |
|:-----|:---------|:-----|
| **复杂任务**（跨 3+ 文件、新模块、架构改动） | `opencode-go/glm-5.2` | 深度推理强，适合设计+实现一体化。实测 78K tokens/10文件/823行/$0.023 ✅ |
| **中等任务**（单文件、逻辑有一定复杂度） | `opencode-go/deepseek-v4-pro` | 稳定可靠，复杂分析首选 |
| **简单任务**（<30 行改、纯配置、测试补全） | `opencode-go/deepseek-v4-flash` | 低成本快速响应 |

如果用户明确指定了模型（如"用 GLM-5.2"），**无条件照做**，不要用上表覆盖用户的选择。

#### 🔴 复杂任务 → 推荐 glm-5.2 或 deepseek-v4-pro
